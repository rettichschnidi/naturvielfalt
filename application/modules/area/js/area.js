...
function Area() {
	...
	google.maps.event.addDomListener(window, "load", function() {
		...
		/*
		 * jQuery("#habitat").autocomplete({ source: "area-create/get-habitats",
		 * focus: function( event, ui ) { console.info(ui); jQuery( "#habitat"
		 * ).val( ui.item.label+" "+ui.item.name ); return false; }, select:
		 * function(event, ui) { jQuery('#habitat').val(ui.item.label+"
		 * "+ui.item.name);
		 * jQuery('#habitat_id_'+me.habitatsCnt).val(ui.item.id);
		 * me.setHabitat(ui.item); return false; }, minLength: 1, change:
		 * function (event, ui) { console.info('change'); //if the value of the
		 * textbox does not match a suggestion, clear its value if
		 * (jQuery(".ui-autocomplete li:textEquals('"+
		 * jQuery(this).val().replace(/([{}\(\)\^$&.\*\?\/\+\|\[\\\\]|\]|\-)/g,
		 * '\\$1') + "')").size() == 0) { jQuery(this).val('');
		 * jQuery('#habitat_id_'+me.habitatsCnt).val(''); } else {
		 * me.setHabitat(ui.item); } } }).live('keydown', function (e) { var
		 * keyCode = e.keyCode || e.which; //if TAB or RETURN is pressed and the
		 * text in the textbox does not match a suggestion, set the value of the
		 * textbox to the text of the first suggestion if((keyCode == 9 ||
		 * keyCode == 13)) { if(jQuery(".ui-autocomplete li:textEquals('"+
		 * jQuery('#habitat').val().replace(/([{}\(\)\^$&.\*\?\/\+\|\[\\\\]|\]|\-)/g,
		 * '\\$1') + "')").size() == 0) { jQuery(this).val('');
		 * jQuery('#habitat_id_'+me.habitatsCnt).val('');
		 *  } else { var item =
		 * jQuery("#habitat").data('autocomplete').selectedItem;
		 * me.setHabitat(item); jQuery('#habitat').focus(); } } }).data(
		 * "autocomplete" )._renderItem = function( ul, item ) { //highlighting
		 * of matches var re_label = new RegExp("^"+this.term, 'i'); var term =
		 * this.term.replace(/[aou]/, function (m){ // to highlight with term
		 * 'wasser' -> 'wasser' and 'gewässer' var hash = { 'a' : '(ä|a)', 'o' :
		 * '(ö|o)', 'u' : '(ü|u)' }; return hash[m]; }); var re_name = new
		 * RegExp(term, 'ig'); var label = item.label.replace(re_label ,"<span
		 * class='ui-term-match'>$&</span>"); var name =
		 * item.name.replace(re_name,"<span class='ui-term-match'>$&</span>");
		 * 
		 * return jQuery( "<li></li>" ) .data( "item.autocomplete", item )
		 * .append( "<a><div class='ui-menu-item-label'>" + label + "</div><div
		 * class='ui-menu-item-name'>" + name + "</div></a>" ) .appendTo( ul ); };
		 */
	});
...
Area.prototype.setHabitat = function(item) {
	...
	habitat_table.append("<tr id='habitat-tr_" + this.habitatsCnt + "'><td>" + item.label + "</td><td>" + item.name
			+ "</td><td><img id='habitat-del_" + this.habitatsCnt + "' src='images/can_delete.png' /></td></tr>");

	jQuery('#habitat-del_' + this.habitatsCnt).bind('click', {
		'entry' : this.habitatsCnt
	}, (function(e) {
