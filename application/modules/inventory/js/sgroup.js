(function($) {
  ...
  function sgroupAutocompleteSelect(event) {
    ...
    if ($.inArray(newSgroupArray[0], sgroupIds) < 0) {
      ...
      var write = "<td>" +
                "<input type='checkbox' class='form-checkbox' name='write_new_" + cnt + "' value='1'/>" +
                "<input type='hidden' name='sgroup_id_" + cnt + "' value='" + newSgroupArray[0] + "'/>" +
              "</td>";
      var read = "<td><input type='checkbox' class='form-checkbox' name='read_new_" + cnt + "' value='1'/></td>";
      ...
      var remove = "<td class='delete'><img src='" + removeImg + "' style='cursor:pointer'/></td>";
      var row = $("<tr>" + name + write + read + show_red + remove + "</tr>");
  ...
  $(document).ready(function() {
    ...
    $("input#edit-sgrouptext").bind("autocomplete_select", sgroupAutocompleteSelect);
    
    $('#inventory-edit-rights table tbody tr').each(initializeGroupRow);
  });
