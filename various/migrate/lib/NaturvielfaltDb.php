...
class NaturvielfaltDb extends Db {
	public function __construct() {
		global $config;
		parent::__construct($config['naturvielfalt_dev']['driver'], $config['naturvielfalt_dev']['name'], $config['naturvielfalt_dev']['user'], $config['naturvielfalt_dev']['password'], $config['naturvielfalt_dev']['host']);
	}
	...
	function createScientificName($organism_id, $scientific_name_name) {// update the organism_scientific_name table
		...
		$numrow = $this -> insert_query($columnArray, $table, $typesArray, $valuesArray);
		assert($numrow == 1);
	...
	function getScientificNameOrganismId($scientific_name_name) {
		global $drupalprefix;
		$table = $drupalprefix . 'organism_scientific_name';
		$columnNameArray = array('organism_id');
	...
	function haveClassifier($classifier_name) {
		global $drupalprefix;
		$fromQuery = 'FROM ' . $drupalprefix . 'organism_classifier
						WHERE
							name = ?';
		$typesArray = array('text');
		$valuesArray = array($classifier_name);
		$num = $this -> getcount_query($fromQuery, $typesArray, $valuesArray);
		assert($num <= 1);
		return $num;
	}
	...
	function createClassifier($classifier_name, $scientific_classification) {
		...
		$rowcount = $this -> insert_query($columnArray, $table, $typesArray, $valuesArray);
		assert($rowcount == 1);
		$ids = $this -> getIdArray_query($columnArray, $table, $typesArray, $valuesArray);
		assert(count($ids) == 1);
		return $ids[0];
	...
	function haveClassificationLevel($classification_level_name, $classifier_id) {
		...
		$fromQuery = 'FROM ' . $table . ' WHERE name = ? AND organism_classifier_id = ?';
		$typesArray = array(
				'text',
				'integer'
		);
	...
	function haveClassificationLevelWithSpecialParentId($classification_level_name, $classification_parent_id, $classifier_id) {
		global $drupalprefix;
		$table = $drupalprefix . 'organism_attribute';
		$fromQuery = 'FROM ' . $table . ' WHERE name = ? AND parent_id = ? AND organism_classifier_id = ?';
		$typesArray = array(
				'text',
				'integer',
				'integer'
	...
	function createClassificationLevel($classification_level_name, $classification_parent_id, $classifier_id) {
		global $drupalprefix;
		$table = $drupalprefix . 'organism_classification_level';
		$newid = $this -> get_nextval($drupalprefix . 'organism_classification_level_id_seq');
		if ($classification_parent_id != 0) {
			$parentLeftValue = $this -> getSingleValue('left_value', $table, $classification_parent_id);
			$parentRightValue = $this -> getSingleValue('right_value', $table, $classification_parent_id);
			...
			$leftValue = 1;
			$rightValue = 2;
			$classification_parent_id = $newid;
		}
	...
	function getClassificationLevelId($classification_level_name, $classifier_id) {
		global $drupalprefix;
		$table = $drupalprefix . 'organism_classification_level';
		$columnArray = array(
				'name',
	...
	function haveClassification($classification_name, $classification_level_id) {
		...
		$fromQuery = 'FROM ' . $table . ' WHERE name = ? AND organism_classification_level_id = ?';
		$typesArray = array(
				'text',
				'integer'
		);
	...
	function createClassification($classification_name, $classification_level_id) {
		assert($classification_name != NULL && $classification_name != '');
		assert($classification_level_id != 0);
		global $drupalprefix;
		$table = $drupalprefix . 'organism_classification';
		$newid = $this -> get_nextval($drupalprefix . 'organism_classification_id_seq');
		$columnArray = array(
				'id',
				'name',
				'organism_classification_level_id'
	...
	function getPrimeFatherId($prime_father_id) {
		...
		$columnNameArray = array('prime_father_id');
		$fromQuery = 'FROM ' . $table . ' WHERE id = ?';
		$typesArray = array('integer');
		$valuesArray = array($prime_father_id);
		$ids = $this -> select_query($columnNameArray, $fromQuery, $typesArray, $valuesArray);
		assert(count($ids) == 1);
	...
	function createOrganism($prime_father_id, $parent_id) {
		...
		if ($parent_id != NULL) {
			$parentLeftValue = $this -> getSingleValue('left_value', $table, $parent_id);
			$parentRightValue = $this -> getSingleValue('right_value', $table, $parent_id);
			$parentPrimeFatherId = $this -> getSingleValue('prime_father_id', $table, $parent_id);
	...
	function haveAttributeName($attribute_name) {
		...
		$fromQuery = 'FROM ' . $table . ' WHERE name = ?';
		$typesArray = array('text');
		$valuesArray = array($attribute_name);
		$num = $this -> getcount_query($fromQuery, $typesArray, $valuesArray);
		assert($num <= 1);
		return $num;
	}
	...
	function haveAttributeValueNumber($attribute_id, $number_value) {
		...
		$typesArray = array('integer');
		$valuesArray = array($number_value);
		$num = $this -> getcount_query($fromQuery, $typesArray, $valuesArray);
		assert($num <= 1);
		return $num;
	}
	...
	function createAttributeValueNumber($attribute_id, $number_value) {
		...
		global $drupalprefix;
		$table = $drupalprefix . 'organism_attribute_value';
		$newid = $this -> get_nextval($drupalprefix . 'organism_attribute_value_id_seq');
		$columnArray = array(
				'id',
	...
	function createAttributeValueSubscription($organism_id, $attribute_value_id) {
		...
		$newid = $this -> get_nextval($drupalprefix . 'organism_attribute_value_subscription_id_seq');
		$columnArray = array(
				'id',
				'organism_id',
	...
	function createOrganismClassificationSubscription($organism_id, $classification_id) {
		assert($organism_id != NULL);
	...
	function haveOrganismClassificationSubscription($organism_id, $classification_id) {
		assert($organism_id != NULL);
		assert($classification_id != NULL);
		global $drupalprefix;
		$table = $drupalprefix . 'organism_classification_subscription';
		$fromQuery = 'FROM ' . $table . ' WHERE organism_id = ? AND organism_classification_id = ?';
		$typesArray = array('integer', 'integer');
		$valuesArray = array($organism_id, $classification_id);
		$num = $this -> getcount_query($fromQuery, $typesArray, $valuesArray);
		assert($num <= 1);
		return $num;
	}
