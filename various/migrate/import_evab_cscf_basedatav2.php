...
$classifier->addOrganism(
		array('classifications' => array(array('classificationlevelname' => 'class'),
						array('classificationlevelname' => 'order'),
						array('classificationlevelname' => 'family'),
						array('classificationlevelname' => 'genus'),
						array('classificationlevelname' => 'subgenus'),
						array('classificationlevelname' => 'species'),
						array('classificationlevelname' => 'subspecies')
				)
		));
...
foreach ($organisms as $organism) {
	if (++$i % 100 == 0) {
		...
		$start = $current;
		$classifier->stopTransactionIfPossible();
		$classifier->startTransactionIfPossible();
	}
	$classifier->addOrganism($organism);
}
$classifier->stopTransactionIfPossible();
exit();
 ...
 */
{
	...
	foreach ($rows as $row) {
		...
		if ($organism_subgenus) {
			...
			if (false)
				print
					("GOT SUBGENUS ID: $organism_classification_id for organism '$organism_scientific_name'\n");
		} else {
