 ...
 * @Author 2012, Reto Schneider github@reto-schneider.ch
 *
 * Converts a DLL-Schema (*.sql, PostgreSQL), generated by ERMaster (ermaster.sf.net)
 * into a Drupal Schema (Schema API)
 *
 * WARNING: its a quick hack and does what it need and that's it - its not nice,
 * it's not flexible and there are no comments. Take it or leave it.
 */
...
function getColumnType(&$column, &$line) {
...
function handleCreateTable(&$schema, &$input, &$line) {
	if (preg_match('/^CREATE\ TABLE\ ([a-z]*\.)?([a-z_]+)$/', $line, $match) == 1) {
		...
		while ($line = getNextLine($input)) {
			...
			if (getColumnType($schema[$tabName]['fields'][$lastcolumn], $line)) {
				continue;
			}
