...
/**
 * @file open_identification_list.tpl.php
 */
$string = "";
if ($list) {
	foreach ($list as $typerecordkey => $typerecord) {
		...
		foreach ($typerecord as $grouprecordkey => $grouprecord) {
			...
			$string .= '<div class="group"><div class="grouptitle">' . t($grouptitle)
					. '</div><div class="grouprecords">';
			foreach ($grouprecord as $record) {
				...
				$string .= '<a href="'
						. ($record['info']['solved'] ? url(
										$record['info']['type'] . '/' . $record['info']['solved_item_id']) : url($link))
						. '">';
				...
				$string .= '<div class="title">'
						. ($record['info']['solved'] ? 'Identified as '
										. open_identification_get_item_name($record['info']['type'],
												$record['info']['solved_item_id'],
												$record['info']['solved_organismgroupid']) . ' on '
										. date("d.m.y", strtotime($record['info']['solved_date']))
								: t('Uploaded by ') . $record['info']['fullname'] . t(' on ')
										. ('' . date("d.m.y", strtotime($record['info']['modified_date'])))) . '</div>';
