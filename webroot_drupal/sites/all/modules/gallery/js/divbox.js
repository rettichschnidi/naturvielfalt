...
(function($) {
	$.fn.divbox = function(opt, clickObj) {
		var _cfg = {
			...
			skin : null,
			speed : 500,
			animate : 'explore', // currently we are only support: default &  explore & scroll & none
			left : null,
			top : null,
			type : null,
			src : 'href',
		...
		function _run(index, init) {
			...
			fn.init = function() {
				...
				$(requires).remove();
				$(_cfg.container).append('<div id="divbox">' + '</div>' + '<div id="divbox_ajax">' + '</div>' + '<div id="divbox_frame" class="' + (_cfg.skin ? _cfg.skin : 'divbox_default') + '">' + '<div class="closed" title="' + _cfg.languages.btn_close + '">' + '</div>' + '<div id="divbox_data">' + '<div id="divbox_content">' + '</div>' + '<div class="prev" title="' + _cfg.languages.btn_prev + '">' + '</div>' + '<div class="next" title="' + _cfg.languages.btn_next + '">' + '</div>' + '</div>' + '<div class="divbox_media_details">' + '</div>' + '</div>');
			...
			fn.main = function(t, l, w, h, fncallback, fnclosed, caption) {
				...
				if(_cfg.caption != false && (caption != '' || _cfg.caption_control == true)) {
					...
					if(_cfg.caption_control == true) {// caption control
						var btn_top = h + parseInt(c.css('padding-top'));
						// 12 = 1/2 height of button prev/next icon
			...
			fn.viewMediaData = function(src, caption) {

				$('div.divbox_media_details').hide();
				$('div.divbox_media_details').empty();
				//var cap = $(oMatch).next().next().clone();
				//cap     = captionFactory(cap);
				var cap;
				cap = $(obj).next().next().clone();

				if(cap.length) {
					var tmp = cap;
					tmp.find('form').each(gallery_rating.stars);
					$('div.divbox_media_details').append(tmp).show();
					tmp.show();
					//$('div.divbox_media_details').slideDown(1300);
					$('div.divbox_media_details').css('display', 'block');
				}
			}
