 ...
 * 
 * Data with Label fixed by Ramon Gamma, ramon@gamma.li
 * 2012-02-07
 * - function addData: replaced with original (From Flexigrid v1.1)
 * - tableId induvidual definied, same as tableid
 * 
 */
...
(function($) {
	...
	$.addFlex = function(t, p) {
			...
			dragEnd : function() {
				...
				if (this.colresize) {
					...
					if (p.cookies) {
						...
						prefs['colsize'] = (typeof prefs.colsize == 'undefined') ? {}
								: prefs.colsize;
		...
		if (p.colModel) {
			...
			for (i = 0; i < p.colModel.length; i++) {
				...
				if (p.cookies) {
					var prefs = g.prefs.load();
					if (typeof prefs.colsize != 'undefined'
							&& typeof prefs.colsize['col_' + i] != 'undefined')
						$(th).attr('width', prefs.colsize['col_' + i]);
				}
	...
	$.fn.recalcLayout = function() { // function to recalculate the layout
										// (refresh)
		return this.each(function() {
			if (this.grid)
