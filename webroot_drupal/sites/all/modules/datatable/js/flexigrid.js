 ...
 *  * THIS FILE IS MAINTAINED HERE: https://github.com/naturwerk/Flexigrid/tree/naturwerk *
 *  * HAVING A SEPARATED REPOSITORY ALLOWS US TO PULL IN CHANGES FROM VANILLA FLEXIGRID   *
 *  * (URL: https://github.com/paulopmx/Flexigrid) VERY EASILY.                           *
 *  ***************************************************************************************
 */
(function($) {
		...
		var g = {
			...
			doSearch : function() {
				...
				{ 
				   ...
				   p.qtype[i] = $('select[name=qtype' + i + ']', g.sDiv).val();
				   ++i;
				   vorcheck = $('input[name=q' + i + ']', g.sDiv).val();
				}
//           cookies wurden nicht berücksichtigt beim Einbau der Suche, ist eh auf false
				if (p.cookies) {
					var prefs = g.prefs.load();
					if (sqtype !== "") {
						prefs.qtype = {};
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$('#flexi_reset', g.sDiv).click(function() {
					...
					$('input[name=q0]', g.sDiv).val('');
					$('input[name=q1]', g.sDiv).val('');
					$('input[name=q2]', g.sDiv).val('');
					$('input[name=q3]', g.sDiv).val('');
					p.query = ['','','',''];					
//				    Gruppenfelder auch mit Space versehen ggg
					$('select[name=qtype0]', g.sDiv).val('');
		...
		if (p.title) {
			...
			if (p.showTableToggleBtn) {
				$(g.mDiv).append(
						'<div class="ptogtitle" title="' + p.minmax
								+ '"><span></span></div>');
